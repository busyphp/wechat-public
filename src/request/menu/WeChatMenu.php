...
class WeChatMenu extends WeChatPublicBaseRequest
{
     ...
     * @return array|string
     */
    public static function getTypes($const = null)
    {
        $array = [
            self::TYPE_VIEW               => '跳转URL',
            self::TYPE_CLICK              => '点击推事件',
            self::TYPE_SCAN_CODE_PUSH     => '扫码',
            self::TYPE_SCAN_CODE_WAIT_MSG => '扫码并等待系统回复',
            self::TYPE_PIC_SYS_PHOTO      => '系统拍照',
            self::TYPE_PIC_PHOTO_OR_ALBUM => '拍照或者相册',
            self::TYPE_PIC_WEIXIN         => '微信相册',
            self::TYPE_LOCATION_SELECT    => '地理位置',
            self::TYPE_MEDIA_ID           => '下发微信素材',
            self::TYPE_VIEW_LIMITED       => '跳转微信图文URL',
            self::TYPE_MINI_PROGRAM       => '跳转到小程序',
        ];
        if (is_null($const)) {
            return $array;
        }
